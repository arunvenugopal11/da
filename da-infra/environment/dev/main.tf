# Dev environment — all module instantiations live here.
...
# ── 2. Common infrastructure (VPC data, security groups, VPC endpoints) ──────
module "common" {
...
module "sqs_notification" {
  ...
  # Producer and consumer roles wired in after lambda_api creates them
  producer_role_arns = [
...
module "sqs_matching" {
  ...
  producer_role_arns = [
    module.lambda_api.execution_role_arns["profile"],
  ]
  consumer_role_arns = [
    module.lambda_api.execution_role_arns["matching"],
  ]
...
module "lambda_api" {
  ...
  # Secrets from HCP Vault — passed through CI/CD as TF_VAR_*
